...
describe('VCalendar/util/timestamp.ts', () => {
  ...
  it('should parse timestamp and update relative flags', () => {
    expect(
      parseTimestamp('2019-01-03', true, parseTimestamp('2019-02-08'))
    ).toMatchObject({
      past: true,
      present: false,
      future: false
    ...
    expect(
      parseTimestamp(
        '2019-01-03 07:00',
        true,
        parseTimestamp('2019-01-03 07:00')
      )
    ).toMatchObject({
      past: false,
      present: true,
  ...
  it('should copy timestamp', () => {
    ...
    const b = copyTimestamp(a as CalendarTimestamp)
    expect(a).not.toBe(b)
    expect(a).toEqual(b)
  })
  ...
  it('should create interval list', () => {
    expect(
      createIntervalList(parseTimestamp('2019-02-08'), 30, 15, 10)
    ).toMatchSnapshot()
    expect(
      createIntervalList(parseTimestamp('2019-02-08'), 15, 15, 10)
    ).toMatchSnapshot()
    expect(
      createIntervalList(parseTimestamp('2019-02-08'), 10, 5, 2)
    ).toMatchSnapshot()
  })
  ...
  //   expect(
  //     createNativeLocaleFormatter('en-US', () => {})(
  //       parseTimestamp('2019-02-08')
  //     )
  //   ).toBe('2/8/2019')
  //   expect(
  //     createNativeLocaleFormatter('en-UK', () => {})(
  //       parseTimestamp('2019-02-08')
  //     )
  //   ).toBe('2/8/2019')
  //   expect(
  //     createNativeLocaleFormatter('ru-RU', () => {})(
  //       parseTimestamp('2019-02-08')
  //     )
  //   ).toBe('2019-2-8')
  // })
  ...
  //   expect(
  //     createNativeLocaleFormatter('', () => {})(parseTimestamp('2019-02-08'))
  //   ).toBe('')
  //   global.Intl = intl
  // })
  ...
  it('should get month start', () => {
    expect(getStartOfMonth(parseTimestamp('2019-02-08')).date).toEqual(
      '2019-02-01'
    )
    expect(getStartOfMonth(parseTimestamp('2019-03-08')).date).toEqual(
      '2019-03-01'
    )
    expect(getStartOfMonth(parseTimestamp('2019-06-08')).date).toEqual(
      '2019-06-01'
    )
  })

  it('should get month end', () => {
    expect(getEndOfMonth(parseTimestamp('2019-02-08')).date).toEqual(
      '2019-02-28'
    )
    expect(getEndOfMonth(parseTimestamp('2019-03-08')).date).toEqual(
      '2019-03-31'
    )
    expect(getEndOfMonth(parseTimestamp('2019-06-08')).date).toEqual(
      '2019-06-30'
    )
  })
  ...
  it('should get next minutes', () => {
    expect(
      nextMinutes({ hour: 8, minute: 30 } as CalendarTimestamp, 40)
    ).toEqual({
      hour: 9,
      minute: 10
    })
    expect(
      nextMinutes({ hour: 8, minute: 10 } as CalendarTimestamp, 90)
    ).toEqual({
      hour: 9,
      minute: 40
    })
    expect(
      nextMinutes({ hour: 8, minute: 0 } as CalendarTimestamp, 40)
    ).toEqual({
      hour: 8,
      minute: 40
    })
    expect(
      nextMinutes(
        { day: 1, weekday: 1, hour: 23, minute: 50 } as CalendarTimestamp,
        40
      )
    ).toEqual({ hour: 0, minute: 30, day: 2, weekday: 2 })
  })

  it('should update minutes', () => {
    expect(updateMinutes({} as CalendarTimestamp, 40)).toMatchObject({
      hour: 0,
      minute: 40
    })
    expect(updateMinutes({} as CalendarTimestamp, 90)).toMatchObject({
      hour: 1,
      minute: 30
    })
    expect(
      updateMinutes({} as CalendarTimestamp, 40, {} as CalendarTimestamp)
    ).toMatchObject({
      hour: 0,
      minute: 40
    })
    expect(
      updateMinutes({} as CalendarTimestamp, 90, {} as CalendarTimestamp)
    ).toMatchObject({
      hour: 1,
      minute: 30
    })
  ...
  it('should get weekday skips', () => {
    expect(getWeekdaySkips([0, 1, 2, 3, 4, 5, 6])).toEqual([
      1,
      1,
      1,
      1,
    ...
    expect(getWeekdaySkips([1, 5, 0, 3, 4, 2, 6])).toEqual([
      1,
      1,
      1,
      1,
    ...
    expect(getWeekdaySkips([1, 5, 1, 3, 4, 2, 6])).toEqual([
      0,
      1,
      1,
      1,
  ...
  describe('createDayList', () => {
    it('should create day list', () => {
      ...
      expect(() =>
        createDayList(
          parseTimestamp('2019-02-02'),
          parseTimestamp('2019-02-01'),
          parseTimestamp('2019-02-02'),
          skips
        )
      ).toThrow()
    ...
    it('should throw error if no days are available given specified weekday skips', () => {
      const skips = getWeekdaySkips([1, 2, 3, 4, 5])
      // 2019-01-12 is a saturday (weekday 6)
      expect(() =>
        createDayList(
          parseTimestamp('2019-01-12'),
          parseTimestamp('2019-01-12'),
          parseTimestamp('2019-02-01'),
          skips
        )
  ...
  it('should find weekday', () => {
    expect(
      findWeekday(parseTimestamp('2019-02-03'), 5, nextDay, 100)
    ).toMatchObject({ weekday: 5 })
    expect(
      findWeekday(parseTimestamp('2019-01-03'), 1, nextDay, 100)
    ).toMatchObject({ weekday: 1 })
    expect(
      findWeekday(parseTimestamp('2019-01-03'), 5, prevDay, 100)
    ).toMatchObject({ weekday: 5 })
    expect(
      findWeekday(parseTimestamp('2019-01-03'), 1, prevDay, 100)
    ).toMatchObject({ weekday: 1 })
  })

  it('should calculate relative days', () => {
    expect(
      relativeDays(parseTimestamp('2019-02-03'), nextDay, 1)
    ).toMatchObject({ day: 4 })
    expect(
      relativeDays(parseTimestamp('2019-01-03'), nextDay, 10)
    ).toMatchObject({ day: 13 })
    expect(
      relativeDays(parseTimestamp('2019-01-03'), nextDay, 1000)
    ).toMatchObject({ day: 29, month: 9, year: 2021 })
    expect(
      relativeDays(parseTimestamp('2019-01-03'), prevDay, 1)
    ).toMatchObject({ day: 2 })
    expect(
      relativeDays(parseTimestamp('2019-01-03'), prevDay, 10)
    ).toMatchObject({ day: 24 })
    expect(
      relativeDays(parseTimestamp('2019-01-03'), prevDay, 1000)
    ).toMatchObject({ day: 8, month: 4, year: 2016 })
  })
})
