...
type CalendarTimestampFormatOptions = (
  timestamp: CalendarTimestamp,
  short: boolean
) => object
type CalendarTimestampOperation = (
  timestamp: CalendarTimestamp
) => CalendarTimestamp
export type VTime =
  | number
...
export function parseTime(input: any): number | false {
  if (typeof input === 'number') {
    ...
    return parseInt(parts[1]) * 60 + parseInt((parts[3] || 0) as string)
...
export function parseTimestamp(
  ...
  if (typeof input !== 'string') {
    if (required) {
      throw new Error(
        `${input} is not a valid timestamp. It must be a Date, number of seconds since Epoch, or a string in the format of YYYY-MM-DD or YYYY-MM-DD hh:mm. Zero-padding is optional and seconds are ignored.`
      )
    }
    return null
  }
...
export function getTimestampIdentifier(timestamp: CalendarTimestamp): number {
  return (
    getDayIdentifier(timestamp) * OFFSET_TIME + getTimeIdentifier(timestamp)
  )
}
...
export function createNativeLocaleFormatter(
  ...
  const emptyFormatter: CalendarFormatter = () => ''
