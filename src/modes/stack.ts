...
function calculateBounds(nodes: Node[], overlapThreshold: number) {
  for (const node of nodes) {
    ...
    const columnWidth =
      (spaceWidth / (columns - node.index + (node.sibling ? 1 : 0))) *
...
function getColumnWidthMultiplier(node: Node, nodes: Node[]): number {
  ...
  const minColumn = node.children.reduce(
...
function getOverlappingRange(
  ...
  for (const other of nodes) {
    if (
      ...
      hasOverlap(node.start, node.end, other.start, other.end)
    ) {
      overlapping.push(other)
    }
  }
  if (returnFirstColumn && overlapping.length > 0) {
    const first = overlapping.reduce(
      (min, n) => Math.min(min, n.index),
      overlapping[0].index
    )
